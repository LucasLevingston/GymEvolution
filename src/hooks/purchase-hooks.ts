...
export function usePurchases() {
  ...
  const getPurchasesByProfessionalId = useCallback(async (): Promise<Purchase[]> => {
    try {
      setIsLoading(true)
      setError(null)

      const { data } = await api.get(`/purchases/professional/${user?.id}`)
      console.log(data)

      return data
    } catch (err: any) {
      const errorMessage = err.response?.data?.error || 'Falha ao buscar pagamentos'
      setError(errorMessage)
      toast.error(errorMessage)
      return []
    } finally {
