import { Peso } from './../interfaces/user.interface';
...
class UserRepositoryPrisma implements UserRepository {
  ...
  async alterarDado(
    email: string,
    field: string,
    novoValor: string | object | Peso | SemanaDeTreinoCreate
  ): Promise<{ field: string; novoValor: string | object } | null> {
    const user = await this.findByEmail(email);
    ...
    if (field === 'semanaDeTreino' && typeof novoValor === 'object') {
      ...
      if ('treino' in novoValor && Array.isArray(novoValor.treino)) {
        ...
        throw new Error(
          'O tipo de dado fornecido não é válido para uma nova semana de treino'
        );
      }
