...
export const Treinando: React.FC = () => {
   ...
   const handleSerieFeita =
      async (nome: string, quantidadeSeries: number,) => {
                     ...
                     <div className='space-y-5 text-preto bg-branco p-3 rounded w-[300px] h-[350px]' key={i}>
                        ...
                        {fazendoExercicio && serieAtual.indexExercicioAtual === i && (
                           <div>
                              ...
                              <div className='text-right' onClick={() => (handleSerieFeita(exercicio.nome, exercicio.quantidadeSeries,))}>
                                 <Button>Feito</Button>
                              </div>
                           </div>
                        )}
                        {exercicio.resultado?.map((resultado, i) =>
                           <div key={i} className='bg-cinza  p-1 rounded'>
                              Série {resultado.serie}: Repetições: {resultado.repeticoes} - Carga: {resultado.carga}kg
                           </div>
                        )}
                     </div>
                  ))}
